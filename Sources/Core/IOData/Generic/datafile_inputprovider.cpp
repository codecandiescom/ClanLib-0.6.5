/*
	$Id: datafile_inputprovider.cpp,v 1.16 2002/02/28 22:12:56 japj Exp $
...
std::string CL_InputSourceProvider_Datafile::get_path(const std::string &filename)
{
	return filename;
}
...
void CL_InputSource_Datafile::seek(int pos, SeekEnum seek_type)
{
	if (compressed)
	{
		...
		else if (seek_type == seek_set)
		{
 		...
 		std::cout << "ClanLib: seek() in datafiles called with an invalid position" << std::endl;
		cl_assert(false);
	}
	else
	{
